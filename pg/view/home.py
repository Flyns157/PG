...
def create_home_screen(root: Tk, user: User):
    ...
    Button(button_frame, text="Ajouter un mot de passe", command=lambda: messagebox.showinfo("Ajout", "Fonctionnalité à implémenter")).pack(side="left", padx=5)
    Button(button_frame, text="Exporter", command=lambda: messagebox.showinfo("Export", "Exportation en cours...")).pack(side="left", padx=5)
    Button(button_frame, text="Afficher/Masquer mots de passe", command=lambda: toggle_password_visibility(tree)).pack(side="left", padx=5)
